...
namespace digioz.Portal.Web.Helpers
{
    public static class Utility
    {
        ...
        public static bool AddLogEntry(string message)
        {
            try
            {
                using (digiozPortalEntities db = new digiozPortalEntities())
                {
                    Log log = new Log();
                    log.Message = message;
        ...
        {
            bool result = false;
            Profile profile = ProfileLogic.GetProfileByEmail(forgotPassword.Email);
        ...
        public static void WriteVisitorSession(string sessionId, string pageUrl, string userName, string ipAddress)
        {
            try
            {
                using (digiozPortalEntities db = new digiozPortalEntities())
                {
                    var prevSession = db.VisitorSessions.SingleOrDefault(x => x.SessionId == sessionId);

        ...
        public ActionResult Process(HttpRequestBase request)
        {
            ...
            switch (cmdName)
            {
                ...
                case "put":
                    if (string.IsNullOrEmpty(target))
                        return Error.MissedParameter(cmdName);
                    parameters = request.QueryString.Count > 0 ?
                    request.Unvalidated.QueryString : request.Unvalidated.Form;
                ...
                case "archive":
                    {
                        ...
                        if (targets == null)
                            Error.MissedParameter("targets");
                        return _driver.Archive(targets);
                    }
                case "extract":
                    {
                        if (string.IsNullOrEmpty(target))
                            return Error.MissedParameter(cmdName);
                        return _driver.Extract(target);
                    }
                default:
                    return Error.CommandNotFound();
            }

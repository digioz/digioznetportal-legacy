...
namespace ElFinder
{
    ...
    public class FileSystemDriver : IDriver
    {
        ...
        JsonResult IDriver.Open(string target, bool tree)
        {
            ...
            if (fullPath.Root.ArchiveMimeTypes.Count() > 0)
            {
                //   answer.Archivers.CreateTypes = fullPath.Root.ArchiveMimeTypes;
                answer.Options.Archivers.Create = fullPath.Root.ArchiveMimeTypes;
            }
            if (fullPath.Root.ExtractMimeTypes.Count() > 0)
            {
                answer.Options.Archivers.Extract = fullPath.Root.ExtractMimeTypes;
            }
        ...
        JsonResult IDriver.Archive(IEnumerable<string> Targets)
        {
            FullPath path = ParsePath(Targets.First());
            string parent = path.IsDirectoty ? path.Directory.Parent.FullName :
                path.File.DirectoryName;
            string ArcName = "";
            if (Targets.Count() == 1)
            {
                FullPath filename = ParsePath(Targets.First());
                if(filename.IsDirectoty)
                {
                    ArcName = filename.Directory.Name + ".zip";
                }
                else
                {
                    ArcName = filename.File.Name.Replace(filename.File.Extension, ".zip");
                    //zip the zip file
                    if (ArcName == filename.File.Name)
                    {
                        ArcName += ".zip";
                    }
                }
            ...
            {
                FullPath filename = ParsePath(hashname);

                if (!filename.IsDirectoty && filename.File.Exists)
                {
                    if (IsVisible(filename.File.FullName))
                        arrFiles.Add(filename.File.FullName);
                }

                if (filename.IsDirectoty && filename.Directory.Exists)
                    arrFiles.AddRange(GetListFiles(filename.Directory.FullName));
            }

            var arrFolder = new List<string>();

            System.IO.FileStream fsOut = System.IO.File.Create(parent + @"\" + ArcName);
            ...
            try
            {
                foreach (string filename in arrFiles)
                {
                    ...
                    var relativePath = fi.DirectoryName.Replace(parent, "").TrimStart(
                        new char[] { '\\' }).Replace("\\", "/") + "/";
            ...
            var output = new ChangedResponse();
            output.Changed.Add((FileDTO)DTOBase.Create(new FileInfo(fsOut.Name), path.Root));
            return Json(output);
        ...
        JsonResult IDriver.Extract(string target)
        {
            FullPath path = ParsePath(target);
            var answer = new AddResponse();

            try
            {
                FastZip fz = new FastZip();
                fz.ExtractZip(path.File.FullName, path.File.DirectoryName, null);
            }
            catch
            {
                return Json(answer);
            }
            var parentDirectory = path.File.Directory;
         //   FullPath path = ParsePath(target);
            ZipFile zf = null;
            try
            {
                zf = new ZipFile(path.File.FullName);

                foreach (ZipEntry zipEntry in zf)
                {
                    var entryName = zipEntry.Name.TrimEnd(new char[]{'/'}).Replace("/", "\\");
                    //only add the dirs and files if current dir;
                    if (entryName.Contains("\\"))
                        continue;

                    if (!zipEntry.IsFile)
                    {
                        DirectoryInfo newDir = new DirectoryInfo(Path.Combine(
                            parentDirectory.FullName,
                            entryName));
                        answer.Added.Add(DTOBase.Create(newDir, path.Root));
                        //           answer.Added.Add((DirectoryInfo)DTOBase.Create(new FileInfo(fsOut.Name), path.Root));           // Ignore directories
                    }
                    else
                    {
                        FileInfo newFile = new FileInfo(Path.Combine(
                            parentDirectory.FullName, entryName));
                        answer.Added.Add(DTOBase.Create(newFile, path.Root));
